...
# --- 1. KONFIGURÁCIA A NAČÍTANIE ASSETOV ---
...
# Tvoj aktuálny Google Apps Script URL
...
def load_assets():
    try:
        ...
        st.error(f"Chyba pri načítaní AI komponentov: {e}")
        return None, None

model, encoders = load_assets()

# --- 2. KOMPLETNÁ DATABÁZA MATERIÁLOV (Tvoje fixné hustoty) ---
MATERIAL_DATA = {
    "PLAST": {
        "PA": 1000, "PP": 1000, "PE-HD": 1000, "PE-UHMW": 1000,
        "PC": 1500, "POM": 1500, "PET-G": 1700, "PVC": 1700, "PEEK": 1400
    },
    "OCEĽ": {
        "1.0037": 7900, "1.0570": 7900, "1.0715": 7900, "1.2379": 7900, "1.7225": 7900
    },
    "NEREZ": {
    ...
    "FAREBNÉ KOVY": {
        "Bronz (CuSn)": 9000, "Meď (Cu)": 9000, "Mosadz (2.0401)": 9000, 
        "Hliník (3.3547)": 2900, "Hliník (3.4365)": 2900, "Titan Gr.5": 4500
    }
}

# --- 3. KOMPLETNÝ ZOZNAM ZÁKAZNÍKOV (Všetci, ktorých si zadala) ---
CUSTOMER_MAP = {
...
# --- 5. UI A LOGIKA ---
st.title("📊 MECASYS: AI Expert (Final Production)")

if model and encoders:
    with st.sidebar:
        st.header("⚙️ Parametre CP")
    ...
    with st.form("input_form"):
        ...
        if st.form_submit_button("➕ PRIDAŤ DO ZOZNAMU"):
            ...
            # Vzorec pre hmotnosť presne podľa Orange Feature Constructor
    ...
    if st.session_state.kosik:
        st.subheader("📋 Položky na spracovanie")
        ...
        if st.button("🚀 GENEROVAŤ CENY MODELOM", type="primary"):
            ...
            for item in st.session_state.kosik:
                # PRÍPRAVA 18 PARAMETROV PRE PREDIKCIU
                row = {
                    ...
                    # Fix chyby: náročnosť ako numeric (číslo)
                    'v_narocnost': float(item['narocnost']), 
                    
                    # Categorical cez encodery
            ...
            st.success("Ceny úspešne vypočítané!")
            ...
            try:
                ...
                st.toast("Dáta uložené do Google tabuľky!")
            except:
                st.error("Nepodarilo sa pripojiť k tabuľke.")
